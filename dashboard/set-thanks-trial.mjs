...
const thanksTrial = process.argv[3];

const allowedThanksTrials = ['none', 'one-year'];

if (!allowedThanksTrials.includes(thanksTrial)) {
  throw `Thanks trial should be one of the following: ${allowedThanksTrials.join(
    ', '
  )}`;
}
...
set(userStaticMetadata, 'thanks_trial', thanksTrial);
